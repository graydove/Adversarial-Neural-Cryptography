...
def _build_Network(plain, key, plainTextLength, keyLength):
    ...
    reshape_Alice_output01 = tf.divide(tf.round(tf.multiply(reshape_Alice_output, y)), y)
    # 定义Bob网络
    with tf.variable_scope('Bob'):
        Bob_input = tf.concat([reshape_Alice_output01, key], axis=1)
    ...
    reshape_Alice_output02 = tf.concat([reshape_Alice_output, key], axis=1)
    # 定义Eve 网络
    with tf.variable_scope('Eve'):
        E_w_1 = tf.Variable(tf.truncated_normal(shape=[plainTextLength + 8, unitsLength], mean=0, stddev=0.1))
    ...
    return reshape_Alice_output,reshape_Alice_output01, Bob_output, Eve_output
