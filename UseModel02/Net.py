...
def _build_Network(plain, key, plainTextLength, keyLength):
    ...
    return reshape_Alice_output,reshape_Alice_output01

def _build_Network01(crypto, key, plainTextLength, keyLength):
    unitsLength = plainTextLength + keyLength
    # 定义Bob网络
    with tf.variable_scope('Bob'):
        Bob_input = tf.concat([crypto, key], axis=1)
    ...
    with tf.variable_scope('Eve'):
        ...
        E_FC_layer1 = tf.nn.sigmoid(tf.matmul(crypto, E_w_1))
