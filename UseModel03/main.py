...
def train():
    ...
    #
    Alice_output = Net._build_Network(plain, key, plainTextLength, keyLength)
    ...
    with tf.Session() as session:
        ...
        with  open('results/alice02_{}.txt'.format(time.time()), 'w') as f:
            f.write(str(res_a02))
        with  open('results/alice03_{}.txt'.format(time.time()), 'w') as f:
            f.write(str(res_a03))
